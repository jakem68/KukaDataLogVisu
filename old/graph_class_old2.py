...
class Graphwindow(QtGui.QDialog):
    ...
    def __init__(self):
        ...
        self.pg = pyqtgraph
        self.win = self.pg.GraphicsWindow()
        self.win.setWindowTitle("Graph Window") #set window title
        self.label = self.pg.LabelItem(justify='right')
        self.win.addItem(self.label)
        self.p1 = self.win.addPlot(row=1, col=0)
        self.p2 = self.win.addPlot(row=2, col=0)
        self.data_list = []

        self.region = self.pg.LinearRegionItem()
    ...
    self.proxy = self.pg.SignalProxy(self.p1.scene().sigMouseMoved, rateLimit=60, slot=mouseMoved)
    ...
    def plot_data(self):
        ...
        for i in range (len(self.data_list)):
            ...
            j += 1
            # myPen = pencolor()
            self.p1.plot(self.data_list[i], pen=myPen)
            self.p2.plot(self.data_list[i], pen=myPen)#, fillLevel=-0.3, brush=(191, 63, 191,50))
